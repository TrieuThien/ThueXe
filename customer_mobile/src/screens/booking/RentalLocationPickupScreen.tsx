...
type Props = NativeStackScreenProps<BookingStackParamList, "RentalLocationPickup">;

export function RentalLocationPickupScreen({ navigation }: Props) {
    ...
    // Auto-fill with current location on first load only
    useEffect(() => {
        if (location.data?.address && !initializedRef.current) {
    ...
    const handleConfirm = () => {
        ...
        navigation.navigate("RentalLocationDropoff", {
            pickupLocation: {
                address: addressInput || selectedAddress,
                ...
                <View style={styles.selectionInfo}>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                        ...
                        <Text style={[styles.addressLabel, { color: theme.colors.text }]} numberOfLines={2}>
                            {isLoadingAddress ? "Đang tải..." : displayAddress || "Nhấn bản đồ để chọn vị trí"}
                        </Text>
                    </View>
                </View>

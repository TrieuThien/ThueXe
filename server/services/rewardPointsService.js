...
function assertAdmin(auth) {
    if (!auth || Number(auth.accountType) !== 3 || auth.role !== "admin") {
        throw new AppError("B?n không có quy?n th?c hi?n thao tác này.", 403, "FORBIDDEN");
    }
}
...
function normalizeInt(value, { fallback = null, min = null } = {}) {
    ...
    if (!Number.isInteger(parsed)) {
        throw new AppError("Giá tr? s? nguyên không h?p l?.", 422, "INVALID_INTEGER");
    }
...
function toDateTimeFromISO(value, endOfDay = false) {
    ...
    if (text.includes("T")) {
        ...
        if (Number.isNaN(date.getTime())) {
            throw new AppError("Th?i gian không h?p l?.", 422, "INVALID_DATE");
        }
        return normalized;
    }
...
function ensureCustomerAccount(wallet) {
    if (!wallet) {
        throw new AppError("Không tìm th?y ngu?i dùng.", 404, "USER_NOT_FOUND");
    }

    if (Number(wallet.account_type) !== 1) {
        throw new AppError("Ch? h? tr? tích di?m cho khách hàng.", 422, "INVALID_USER_TYPE");
    }
}
...
function mapActionTypeLabel(actionType) {
    switch (Number(actionType)) {
        ...
        case ACTION_TYPE.ADJUST_ADD:
            return "Ði?u ch?nh tang";
        case ACTION_TYPE.ADJUST_SUBTRACT:
            return "Ði?u ch?nh gi?m";
        default:
            return "Khác";
    }
}
...
function normalizeConfigPayload(payload = {}) {
    ...
    if (![0, 1].includes(status)) {
        throw new AppError("Tr?ng thái tích di?m ph?i là 0 ho?c 1.", 422, "INVALID_STATUS");
    }
...
function normalizeHistoryFilters(query = {}) {
    ...
    if (actionType !== null && ![1, 2, 4, 5].includes(actionType)) {
        throw new AppError("Lo?i hành d?ng không h?p l?.", 422, "INVALID_ACTION_TYPE");
    }
    ...
    if (dateFrom && dateTo) {
        ...
        if (from.getTime() > to.getTime()) {
            throw new AppError("Kho?ng th?i gian l?c không h?p l?.", 422, "INVALID_DATE_RANGE");
        }
    }
...
async function lockAndGetConfigOrThrow(connection) {
    ...
    if (!config) {
        throw new AppError("Chua c?u hình tích di?m.", 422, "REWARD_CONFIG_NOT_FOUND");
    }
    ...
    if (!Number.isFinite(curToPoints) || curToPoints <= 0) {
        throw new AppError("C?u hình quy d?i ti?n sang di?m không h?p l?.", 422, "INVALID_CUR_TO_POINTS");
    }

    if (!Number.isFinite(pointsToCur) || pointsToCur <= 0) {
        throw new AppError("C?u hình quy d?i di?m sang ti?n không h?p l?.", 422, "INVALID_POINTS_TO_CUR");
    }

    if (!Number.isInteger(minRedeem) || minRedeem < 1) {
        throw new AppError("C?u hình di?m t?i thi?u d?i không h?p l?.", 422, "INVALID_MIN_REDEEM");
    }
...
async function processEarnPointsForBookingInternal(bookingIdInput, actorUserId = null) {
    ...
    if (!bookingId) {
        throw new AppError("booking_id không h?p l?.", 422, "INVALID_BOOKING_ID");
    }
    ...
    try {
        ...
        if (Number(config.status) !== 1) {
            ...
            return {
                ...
                message: "Tích di?m dang t?t nên không c?ng di?m.",
            };
        }

        const booking = await findBookingForRewardByIdForUpdate(bookingId, connection);
        if (!booking) {
            throw new AppError("Không tìm th?y booking.", 404, "BOOKING_NOT_FOUND");
        }
        ...
        if (!(Number(booking.status) === 3 && Number(booking.haspaid) === 1)) {
            ...
            return {
                ...
                booking_id: bookingId,
                message: "Booking chua d? di?u ki?n c?ng di?m (status=3 và haspaid=1).",
            };
        }
        ...
        if (existing) {
            ...
            return {
                ...
                message: "Booking dã du?c c?ng di?m tru?c dó.",
            };
        }
        ...
        if (earnedPoints <= 0) {
            ...
            return {
                ...
                message: "Giá tr? booking chua d? d? nh?n di?m.",
            };
        }
        ...
        await insertRewardHistory(
            {
                ...
                ref_table: "bookings",
                ref_id: booking.id,
                note: "C?ng di?m t? d?ng t? booking hoàn t?t và dã thanh toán",
...
export async function adjustRewardPointsByAdmin(payload, auth) {
    ...
    if (!userId) {
        throw new AppError("user_id không h?p l?.", 422, "INVALID_USER_ID");
    }

    if (!Number.isFinite(points) || points === 0) {
        throw new AppError("points ph?i khác 0.", 422, "INVALID_POINTS");
    }
    ...
    try {
        ...
        if (points > 0) {
            ...
            if (balanceBefore < subtractPoints) {
                throw new AppError("Khách hàng không d? di?m d? tr?.", 422, "INSUFFICIENT_POINTS");
            }
        ...
        await insertRewardHistory(
            {
                ...
                note: note || (actionType === ACTION_TYPE.ADJUST_ADD ? "Ði?u ch?nh tang di?m th? công" : "Ði?u ch?nh gi?m di?m th? công"),
...
export async function redeemRewardPointsByAdmin(payload, auth) {
    ...
    try {
        ...
        if (pointsValue < Number(config.min_points_redeemable)) {
            throw new AppError(
                `Ði?m d?i t?i thi?u là ${config.min_points_redeemable}.`,
                422,
        ...
        if (balanceBefore < pointsValue) {
            throw new AppError("Khách hàng không d? di?m d? d?i.", 422, "INSUFFICIENT_POINTS");
        }
